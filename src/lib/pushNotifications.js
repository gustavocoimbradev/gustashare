...
//
// Só "vigia" salas personalizadas (não as PERMANENT_ROOMS) — e só uma por
// vez: entrar numa sala personalizada nova troca a que você vigiava antes.
//
...
// Chamar ao entrar numa sala: se for personalizada, passa a vigiá-la (no
// lugar de qualquer outra que vigiava antes) e avisa quem já vigiava que
// você entrou. Nunca lança — falha de rede/permissão só significa "sem
// notificação dessa vez", não deve quebrar a entrada na sala.
export async function watchRoomAndAnnounceJoin(roomCode, nickname) {
  if (isPermanentRoomName(roomCode)) return;
